...
policy_actor_state_dict_0 = torch.load(str(cfg.get("algorithm", 'model_dir')) + '/actor_agent' + str(0) + '.pt')
policy_actor_state_dict_1 = torch.load(str(cfg.get("algorithm", 'model_dir')) + '/actor_agent' + str(1) + '.pt')
policy_actor_state_dict_2 = torch.load(str(cfg.get("algorithm", 'model_dir')) + '/actor_agent' + str(2) + '.pt')
policy_actor_state_dict_3 = torch.load(str(cfg.get("algorithm", 'model_dir')) + '/actor_agent' + str(3) + '.pt')
envs.reset()
actor0 = R_Actor(config['cfg'], config['envs'].observation_space[0], config['envs'].action_space[0], config['device'])
actor0.load_state_dict(policy_actor_state_dict_0)

actor1 = R_Actor(config['cfg'], config['envs'].observation_space[0], config['envs'].action_space[0], config['device'])
actor1.load_state_dict(policy_actor_state_dict_1)

actor2 = R_Actor(config['cfg'], config['envs'].observation_space[0], config['envs'].action_space[0], config['device'])
actor2.load_state_dict(policy_actor_state_dict_2)

actor3 = R_Actor(config['cfg'], config['envs'].observation_space[0], config['envs'].action_space[0], config['device'])
actor3.load_state_dict(policy_actor_state_dict_3)
...
while True:
    ...
    # 1
    tmp, _, rnn_states_actor_n[0] = actor0(obs[0], rnn_states_actor_n[0], masks)
    ...
    # 2
    tmp, _, rnn_states_actor_n[1] = actor1(obs[1], rnn_states_actor_n[1], masks)
    ...
    # 3
    tmp, _, rnn_states_actor_n[2] = actor2(obs[2], rnn_states_actor_n[2], masks)
    action_n[2] = int(tmp[0][0])
    rnn_states_actor_n[2] = np.array(np.split(_t2n(rnn_states_actor_n[2]), 1))
    rnn_states_actor_n[2] = np.reshape(rnn_states_actor_n[2], (1, 1, 64))

    # 4
    tmp, _, rnn_states_actor_n[3] = actor3(obs[3], rnn_states_actor_n[3], masks)
    action_n[3] = int(tmp[0][0])
    rnn_states_actor_n[3] = np.array(np.split(_t2n(rnn_states_actor_n[3]), 1))
    rnn_states_actor_n[3] = np.reshape(rnn_states_actor_n[3], (1, 1, 64))
    # action_n[1] = np.array(np.split(_t2n(action_n[1]), 1))
    # action_n[1] = np.reshape(action_n[1], (1, 1))
    action_n = np.reshape(action_n, (4, 1))
    obs, rewards, dones, infos = envs.step(action_n)
    if envs.agents[0].is_crash and envs.agents[1].is_crash and envs.agents[2].is_crash and envs.agents[3].is_crash:
        envs.reset()
        obs = []
        obs.append(envs._get_obs(envs.agents[0]))
        obs.append(envs._get_obs(envs.agents[1]))
